...
You are a specialized agent building ONLY the font_parser component.
...
- ✅ Work within this directory: `components/font_parser/`
...
```python
# components/font_parser/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "font_parser"}
```
...
**Location**: Your work is in `components/font_parser/`
...
```bash
python ../../orchestration/git_retry.py "font_parser" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/font_parser/

# Commit with component prefix
git commit -m "[font_parser] feat: Add new capability"
...
```bash
git status components/font_parser/
# or
python ../../orchestration/git_status.py "font_parser"
```
...
- [ ] Read API contract: `../../contracts/font_parser-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "font_parser" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "font_parser" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/font_parser-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/font_parser
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(font_parser): add input validation for common formats
...
Component sections:
- Phase 2: Font Parser Implementation (lines 387-438)
