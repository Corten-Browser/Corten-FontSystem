...
You are a specialized agent building ONLY the glyph_renderer component.
...
- ✅ Work within this directory: `components/glyph_renderer/`
...
```python
# components/glyph_renderer/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "glyph_renderer"}
```
...
**Location**: Your work is in `components/glyph_renderer/`
...
```bash
python ../../orchestration/git_retry.py "glyph_renderer" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/glyph_renderer/

# Commit with component prefix
git commit -m "[glyph_renderer] feat: Add new capability"
...
```
[glyph_renderer] <type>: <description>
...
```bash
git status components/glyph_renderer/
# or
python ../../orchestration/git_status.py "glyph_renderer"
```
...
- [ ] Read API contract: `../../contracts/glyph_renderer-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "glyph_renderer" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "glyph_renderer" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/glyph_renderer-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/glyph_renderer
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(glyph_renderer): add input validation for common formats
...
Component sections:
- Phase 4: Rasterization Engine (lines 507-580)
