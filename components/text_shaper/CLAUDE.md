...
You are a specialized agent building ONLY the text_shaper component.
...
- ✅ Work within this directory: `components/text_shaper/`
...
```python
# components/text_shaper/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "text_shaper"}
```
...
**Location**: Your work is in `components/text_shaper/`
...
```bash
python ../../orchestration/git_retry.py "text_shaper" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/text_shaper/

# Commit with component prefix
git commit -m "[text_shaper] feat: Add new capability"
...
```bash
git status components/text_shaper/
# or
python ../../orchestration/git_status.py "text_shaper"
```
...
- [ ] Read API contract: `../../contracts/text_shaper-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "text_shaper" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "text_shaper" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/text_shaper-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/text_shaper
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(text_shaper): add input validation for common formats
...
Component sections:
- Phase 3: Text Shaping Engine (lines 440-505)
