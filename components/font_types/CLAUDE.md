...
You are a specialized agent building ONLY the font_types component.
...
### Purpose
Common types, traits, enums, and interfaces for the font system
...
- ✅ Work within this directory: `components/font_types/`
...
```python
# components/font_types/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "font_types"}
```
...
**Location**: Your work is in `components/font_types/`
...
```bash
python ../../orchestration/git_retry.py "font_types" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/font_types/

# Commit with component prefix
git commit -m "[font_types] feat: Add new capability"
...
```bash
git status components/font_types/
# or
python ../../orchestration/git_status.py "font_types"
```
...
- [ ] Read API contract: `../../contracts/font_types-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "font_types" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "font_types" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/font_types-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/font_types
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(font_types): add input validation for common formats
