...
**Tech Stack**: Rust, fontconfig (Linux)
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the font_registry component.
...
### Purpose
Font discovery, loading, caching, and font matching algorithms
...
- ✅ Work within this directory: `components/font_registry/`
...
```python
# components/font_registry/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "font_registry"}
```
...
**Location**: Your work is in `components/font_registry/`
...
```bash
python ../../orchestration/git_retry.py "font_registry" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/font_registry/

# Commit with component prefix
git commit -m "[font_registry] feat: Add new capability"
...
```
[font_registry] <type>: <description>
...
```bash
git status components/font_registry/
# or
python ../../orchestration/git_status.py "font_registry"
```
...
- [ ] Read API contract: `../../contracts/font_registry-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "font_registry" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "font_registry" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/font_registry-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/font_registry
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(font_registry): add input validation for common formats
