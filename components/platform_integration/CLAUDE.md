# platform_integration Component
...
You are a specialized agent building ONLY the platform_integration component.
...
- ✅ Work within this directory: `components/platform_integration/`
...
```python
# components/platform_integration/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "platform_integration"}
```
...
**Location**: Your work is in `components/platform_integration/`
...
```bash
python ../../orchestration/git_retry.py "platform_integration" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/platform_integration/

# Commit with component prefix
git commit -m "[platform_integration] feat: Add new capability"
...
```
[platform_integration] <type>: <description>
...
```bash
git status components/platform_integration/
# or
python ../../orchestration/git_status.py "platform_integration"
```
...
- [ ] Read API contract: `../../contracts/platform_integration-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "platform_integration" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "platform_integration" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/platform_integration-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/platform_integration
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(platform_integration): add input validation for common formats
...
**Commit Message Format:**
```
<type>(platform_integration): <subject>
...
Component sections:
- Phase 5: Platform Integration (lines 582-628)
